  ...
  async function elliotGraph() {
    ...
    teams.forEach((teamNumber) => {
      ...
      matchesOfTeam.forEach((matchData) => {
        ...
        if (calculationSuccess && i >= matchesOfTeam.length - 3) {
          values.push(
            totalForMatch *
    ...
    let chartDiv = document.createElement("div");
    chartDiv.id = "Elliots Graph";
    chartDiv.classList.add("chart");
    const overallContainer = document.getElementById("overallGraphContainer");
    overallContainer.appendChild(chartDiv);
    let chartParameters = {
      animationEnabled: false, // Disable animation for potential performance gain/stability
      title: {
        text: "Elliots Metric",
    ...
    try {
      // Drawing graph
      var chart = new CanvasJS.Chart("Elliots Graph", chartParameters);
      chart.render();
      window.renderedCharts.push(chart);
    } catch (error) {
      console.warn(error);
